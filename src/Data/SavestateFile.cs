...
namespace ForestOverlay.Data
{
    ...
    public sealed class SavestateFile
    {
        ...
        /// Null when the file has no enemies line (before v0.24.16);
        /// EnemyRecord entries.
        ...
        /// Game/Stance's value at capture (`crouched` / `standing`); ""
        ...
        public static SavestateFile Parse(string text, out string error)
        {
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                switch (key)
                {
                    ...
                    case "enemies":
                        {
                            s.Enemies = new List<string>();
                            string[] keys = value.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
                            for (int k = 0; k < keys.Length; k++) s.Enemies.Add(keys[k].Trim());
                            break;
                        }
                    case "held":
                        {
